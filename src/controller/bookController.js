...
const createBook = async (req, res) => {
  try {
    ...
    let { title, ISBN, releasedAt, category, userId, subcategory, excerpt } =
      data;
    ...
    if (isEmpty(releasedAt)) {
      ...
      if (!isValidDateFormat(releasedAt))
        return res.status(400).send({
          status: false,
...
const getBookById = async (req, res) => {
  try {
    ...
    // get arr of reviews
    const reviewArr = await reviewsModel
      .find({
        bookId: data._id,
        isDeleted: false,
      })
...
module.exports = {createBook,delBookById,bookUpdate,getBook,getBookById};
