...
const registerUser = async (req, res) => {
  try {
    ...
    if (!isValidRequestBody(data))
      return res
        .status(400)
        .send({ status: false, message: "User credentials are required" });
    ...
    if (!name.match(/^[#.a-zA-Z\s,-]+$/))
      return res
        .status(400)
        .send({ status: false, message: "User name is Invalid !" });
    ...
    if (!isValidPassword(password))
      return res
        ...
        .send({
          status: false,
          message: ` Password ${password} length must be between 8 and 15 and must contain mix of unique character @#$%&* and a-z, A-Z`,
        });

    //DB calls to check valid phone and email
    const isUniquePhone = await UserModel.findOne({ phone: phone }).catch(
      (e) => null
    );
    if (isUniquePhone)
      return res
    ...
    const isUniqueEmail = await UserModel.findOne({ email: email }).catch(
      (e) => null
    );
    if (isUniqueEmail)
      return res
...
const loginUser = async function (req, res) {
  try {
    ...
    if (!isValidRequestBody(loginData))
      return res
        .status(400)
        .send({ status: false, message: "Login Credentials cannot be empty" });
    ...
    let token = jwt.sign(
      ...
      "DHARMENDRAKUMARYADAV-SIWAN-BIHAR-PINCODE-841506"
    );
